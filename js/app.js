(() => {
  ...
  function startMode(nextMode) {
    ...
    if (mode === "learn") {
      ...
      const pool = shuffle(data.questions).slice(0, Math.min(10, data.questions.length));
  ...
  function renderLearn() {
    ...
    activityTitle.textContent = `${data.id} Indigenous Cultures — Learn`;
  ...
  function renderLearnComplete() {
    ...
    activityCard.innerHTML = `
      <div class="result-hero">
        ...
        <h2>You've finished 1.1 Learn.</h2>
        ...
        <div class="actions" style="justify-content:center; flex-wrap:wrap;">
          <button class="secondary" id="goPractice">🧠 Practice 1.1</button>
  ...
  function renderPractice() {
    ...
    activityTitle.textContent = `${data.id} Indigenous Cultures — Practice`;
  ...
  function renderMC() {
    ...
    activityTitle.textContent = `${data.id} Indigenous Cultures — Multiple Choice`;
  ...
  function renderQuestion(q, context) {
    const choices = q.choices.map((choice, index) => {
      ...
      return `<button class="${cls}" data-choice="${index}" ${state.checked ? "disabled" : ""}>${escapeHtml(choice)}</button>`;
    }).join("");
    ...
    activityCard.innerHTML = `
      <div class="result-hero">
        ...
        <div class="actions" style="justify-content:center; flex-wrap:wrap;">
          ...
          <button class="secondary" id="toPractice">Practice 1.1</button>
